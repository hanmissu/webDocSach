	       	...
	       	<?php if( get_theme_mod( 'vw_book_store_grid_postdate',true) == 1 || get_theme_mod( 'vw_book_store_grid_author',true) == 1 || get_theme_mod( 'vw_book_store_grid_comments',true) == 1) { ?>
	            <div class="post-info">
	                ...
	                <?php if(get_theme_mod('vw_book_store_grid_author',true)==1){ ?>
	                    <span><?php echo esc_html(get_theme_mod('vw_book_store_single_post_meta_field_separator', '|'));?></span> <i class="<?php echo esc_attr(get_theme_mod('vw_book_store_grid_author_icon','fas fa-user')); ?>"></i><span class="entry-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span>
	                <?php } ?>

	                <?php if(get_theme_mod('vw_book_store_grid_comments',true)==1){ ?>
	                    <span><?php echo esc_html(get_theme_mod('vw_book_store_single_post_meta_field_separator', '|'));?></span> <i class="<?php echo esc_attr(get_theme_mod('vw_book_store_grid_comments_icon','fa fa-comments')); ?>" aria-hidden="true"></i><span class="entry-comments"><?php comments_number( __('0 Comment', 'vw-book-store'), __('0 Comments', 'vw-book-store'), __('% Comments', 'vw-book-store') ); ?> </span>
	                <?php } ?>
	            </div>
        	<?php } ?>     
	        <div class="new-text">
	          	<div class="entry-content">
	          		<p>
			          <?php $vw_book_store_excerpt = get_the_excerpt(); echo esc_html( vw_book_store_string_limit_words( $vw_book_store_excerpt, esc_attr(get_theme_mod('vw_book_store_related_posts_excerpt_number','30')))); ?> <?php echo esc_html( get_theme_mod('vw_book_store_excerpt_suffix','') ); ?>
			        </p>
	          	</div>
	        </div>
